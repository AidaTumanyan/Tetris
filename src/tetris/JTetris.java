...
public class JTetris extends JComponent {
    ...
    public JTetris(int pixels) {
        ...
        registerKeyboardAction(
                ...
                }, "rotate", KeyStroke.getKeyStroke('s'), WHEN_IN_FOCUSED_WINDOW
        );
        ...
        registerKeyboardAction(
                ...
                }, "drop", KeyStroke.getKeyStroke('x'), WHEN_IN_FOCUSED_WINDOW
        );


        // Create the Timer object and have it send
        // tick(Move.DOWN) periodically
    ...
    public int setCurrent(Piece piece, int x, int y) {
        try {
            ...
            return PLACE_OK;
        } catch (OutOfBoundPlaceException e) {
            //else {
            board.undo();
     ...
     * based on the given verb (Move.LEFT, Move.RIGHT, ...).
     ...
     */
    public void computeNewPosition(Move verb) {
    ...
    //public static final int Move.ROTATE = 0;
    //public static final int Move.LEFT = 1;
    //public static final int Move.RIGHT = 2;
    //public static final int Move.DROP = 3;
    //public static final int Move.DOWN = 4;
	/*
	 Called to change the position of the current piece.
	 Each key press calls this once with the verbs
	 Move.LEFT Move.RIGHT Move.ROTATE Move.DROP for the user moves,
	 and the timer calls it with the verb Move.DOWN to move
	...
	*/
    public void tick(Move verb) {
        ...
        boolean failed = (result >= PLACE_OUT_BOUNDS);
		...
		/*
		 ...
		 if this move hits something on its Move.DOWN verb,
		 and the previous verb was also Move.DOWN (i.e. the player was not
		...
		*/
        if (failed && verb == Move.DOWN && !moved) {    // it's landed
        ...
        // Note if the player made a successful non-Move.DOWN move --
        // used to detect if the piece has landed on the next tick()
        moved = (!failed && verb != Move.DOWN);
    }
